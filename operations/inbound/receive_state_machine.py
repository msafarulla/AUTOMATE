...
class ReceiveStateMachine:
    ...
    def read_screen_text(self) -> str:
        ...
        try:
            return self.rf.rf.read_field("body").lower()
        except Exception:
            return ""
    ...
    def is_element_visible(self, selector: str, timeout: int = 500) -> bool:
        ...
        try:
            rf_iframe = self.rf.rf.get_iframe()
...
class AwaitingBlindIlpnHandler(StateHandler):
    ...
    def execute(self, m: ReceiveStateMachine) -> ReceiveState:
        ...
        for selector in selectors_to_try:
            try:
                has_error, msg = m.rf.rf.fill_and_submit(
...
def _read_suggested_location(m: ReceiveStateMachine) -> str:
    ...
    for key in ('suggested_location_aloc', 'suggested_location_cloc'):
        ...
        if not selector:
            continue
        try:
            loc = m.rf.rf.read_field(selector).replace('-', '').strip()
            if loc:
                return loc
        except Exception:
            continue
