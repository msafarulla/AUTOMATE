...
class TestRFPrimitives:
    ...
    def test_fill_and_submit_success(self, primitives, mock_page):
        """Test successful field fill and submit."""
        # Setup
        mock_input = MagicMock()
        mock_locator = MagicMock()
        mock_locator.first = mock_input
        mock_page.main_frame.locator = MagicMock(return_value=mock_locator)
    ...
    def test_read_field(self, primitives, mock_page):
    ...
    def test_read_field_with_transform(self, primitives, mock_page):
        """Test reading field with transformation."""
        # Setup
        mock_locator = MagicMock()
        mock_locator.inner_text.return_value = "A-01-02"
        mock_page.main_frame.locator = MagicMock(return_value=mock_locator)
    ...
    def test_accept_message(self, primitives, mock_page):
        ...
        # Setup - mock the frame snapshot to return proper string
        with patch('operations.rf_primitives.HashUtils') as mock_hash:
            mock_hash.get_frame_snapshot.return_value = "snapshot_hash"
...
class TestAutoAcceptBehavior:
    ...
    def test_invalid_test_data_not_auto_accepted(self, mock_rf_primitives):
        ...
        # Should have error and sentinel message
        assert has_error
        assert msg == RFPrimitives.INVALID_TEST_DATA_MSG
        # Should not auto-accept due to sentinel check
        mock_rf_primitives.accept_message.assert_not_called()